...
from safety import REFUSAL, is_sensitive, redact, safe_messages, sensitive_user_input
...
def execute_tool_call(call: ToolCall, consent: BookingConsent | None = None) -> dict[str, Any]:
    if is_sensitive(json.dumps(call.args, ensure_ascii=False)) or redact(call.args) != call.args:
        return {"tool": call.name, "args": redact(call.args),
                "result": {"error": "restricted_sensitive_data"}}
    if call.name == "create_booking" and (consent is None or not consent.consume(call.args)):
        return {"tool": call.name, "args": call.args,
                "result": {"error": "runtime_confirmation_required"}}
...
def assistant_tool_message(response_text: str | None, calls: list[ToolCall]) -> dict[str, str]:
    ...
    content = redact(response_text) or "I will call the selected tool(s)."
    return {
        "role": "assistant",
...
def run_model_tool_loop(
    *,
    ...
    model: str | None,
    max_tool_rounds: int,
    consent: BookingConsent | None = None,
) -> dict[str, Any]:
    consent = consent if consent is not None else BookingConsent()
    latest = next((m.get("content", "") for m in reversed(messages) if m.get("role") == "user"), "")
    consent.begin_turn(latest)
    if sensitive_user_input(messages):
        return {"status": "answered", "assistant_text": REFUSAL, "rounds": [],
                "tool_events": [], "safety_guard": "sensitive_input"}
    working_messages = safe_messages(messages)
    rounds: list[dict[str, Any]] = []
    all_tool_events: list[dict[str, Any]] = []

    for round_index in range(1, max_tool_rounds + 1):
        response = provider.complete(working_messages, consent.available_tools(tools), model=model, temperature=0.0)
        ...
        if not calls:
            ...
            return {
                "status": "answered",
                "assistant_text": redact(response.text) or "",
                "rounds": rounds,
        ...
        for call in calls:
            print(f"[tool] {call.name}({json.dumps(redact(call.args), ensure_ascii=True, sort_keys=True)})")
            event = execute_tool_call(call, consent)
